...
\end{frame}   
    ...
    \begin{frame}
        \frametitle{NOOOOOOOOOOOOOOOOO Introduction}
        \begin{center}
            \begin{itemize}
                ...
                \item The search for practical, relevant, and invariant features that convey good-enough information~\footfullcite{Perdikis2014}.
                \item Ethical aspects should be revised~\footfullcite{Yuste2017}.
            \end{itemize}
        \end{center}
    \end{frame} 
...
\begin{itemize}
 ...
 \item<2-> Slope Horizontal Chain Code
 \item<3-> Matching Pursuit
 \item<4-> Merge of Increasing and Decreasing Sequences (REFs!)
...
\begin{frame}
\frametitle{NOOO The Histogram of Gradient Orientations}
\begin{center}
\begin{enumerate}
 \item<1-> Signal Preprocessing
 \item<2-> Signal Segmentation
...
\noindent  where $\mathbf{p}$ is a pixel from within the patch,  $\theta$ is the angle bin with $ \theta \in \{0, 45, 90, 135, 180, 225, 270, 315\} $,  $ \left\lVert J(\mathbf{p}) \right\rVert $ is the norm of the gradient vector in the pixel $\mathbf{p}$, computed using finite differences, and $\angle J(\mathbf{p}) $ is the angle of the gradient vector.  The scalar $ \omega_\mathrm{ang}(\cdot) $  and vector $ \omega_{ij}(\cdot) $ functions are linear interpolations used by~\footfullcite{Lowe2004} and \footfullcite{Vedaldi2010} to provide a weighting contribution to eight adjacent bins. 
\end{center}
\end{frame}
...
\begin{figure}[h!]
\centering
\includegraphics[height=8cm,width=1\textwidth]{images/ClassificationFull.pdf}
...
\begin{figure}[]
\centering
\includegraphics[height=6cm,width=6cm]{images/emotivsubject.jpg}
\includegraphics[height=6cm,width=8cm]{images/eeglab2.jpg}
\caption[Wearable portable Digital Electroencephalograph]{EPOC Emotiv and Alpha Waves signals}
\label{fig:digitalelectroencephalograph}
\end{figure}
\end{frame}
...
\caption{\centering Dataset III: Motor Imagery \footfullcite{Steyrl, Scherer et al 2015}}
\end{figure} 	
\end{frame}	
